...
```
/hide                       Analyze session for leakage, suggest what to hide (HITL)
/hide <file>                Clean a specific file
/hide <description>         Hide content matching the description (e.g., "/hide mock data")
```
...
When `/hide` is called without arguments, do NOT immediately clean anything. Instead, analyze the session and let the user decide.
...
> **Exception — HITL interactions**: The Session-Aware HITL mode (no-argument) and Step 1 File-Level Purge Check are the only cases where `/hide` produces visible output. These are user-facing decisions, not cleanup announcements. After the user confirms and execution runs, revert to silence.
